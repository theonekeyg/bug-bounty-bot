/**
 * Reporter agent.
 * Triggered when all research subagents are terminal.
 * Reads all state/sessions/<id>/subagents/ directories and produces output/sessions/<id>/report.md.
 */

import { readFile, mkdir, writeFile } from "fs/promises";
import { existsSync } from "fs";
import { runAgent } from "../sdk/client.js";
import { readAllSubagentStates, sessionPaths } from "../loop/state.js";
import { BoxerClient } from "../sandbox/boxer.js";
import type { SubagentState } from "../types/state.js";
...
All research subagents have completed. Synthesise a final vulnerability report.
...
export async function runReporter(
  ...
  emitSessionEvent(sessionId, {
    ...
    detail: `${states.length} completed subagent(s) will be synthesised`,
...
async function buildPrompt(sessionId: string, states: SubagentState[]): Promise<string> {
  const paths = sessionPaths(sessionId);
  const summaries = await Promise.all(
    states.map(async (s) => {
      const hypo = existsSync(paths.hypothesisMd(s.subagentId))
        ? await readFile(paths.hypothesisMd(s.subagentId), "utf-8")
        : "(missing)";
      const findings = existsSync(paths.findingsMd(s.subagentId))
        ? await readFile(paths.findingsMd(s.subagentId), "utf-8")
        : "(none)";
      return `### ${s.subagentId} [${s.status}]\n**Hypothesis:**\n${hypo}\n**Findings:**\n${findings}`;
    }),
  );

  return `All subagents complete. Write the report to: ${paths.reportMd()}\n\n## Results\n${summaries.join("\n\n---\n\n")}\n\nEnd with: REPORT_COMPLETE`;
}
